...
void ang_dist_glove_subset()
{
    ...
    for (int m = 1; m < 9; m*=2) {
        ...
        for (int j = 0; j < test_dim.first; j++) {
            for (int i = 0; i < train_dim.first; i++) {
                result_arr[j*train_dim.first + i] = filter.asymmetricDistanceComputation(test[j], train[i]);
            }
        }
        ...
        for (int j = 0; j < test_dim.first; j++) {
            for (int i = 0; i < train_dim.first; i++) {
                result_arr[j*train_dim.first + i]  =  filter.symmetricDistanceComputation(test[j], train[i]);
            }
        }
